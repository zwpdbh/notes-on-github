...
*** code with parallel 
Assume the number of threads is multiple times of NUMA nodes.
#+BEGIN_SRC C
  #include <iostream>
  #include <chrono>
  #include <vector>
  ...
  // by default, access with numa awareness
  int without_numa = 0;
  ...
  int main(int argc, char* argv[]) {
    if (argc != 4) {
      printf("usage: ./xxx <row> <col> <without_numa>\n exit...\n");
      exit(-1);
    }
    ...
    s_each = s / NUM_NODES;
    printf("the size of memory needed to allocate on each node is %lld\n", s_each);


    mx = (float**)emalloc(sizeof(float*) * NUM_NODES);
    
    // make sure to use strick mode
    numa_set_strict(1);
    ...
    for (int i = 0; i < NUM_NODES; i++) {
      tid[i] = i;
      err = pthread_create(&thread[i], NULL, thr_alloc_fn, (void *)&tid[i]);
      if (err != 0) {
    ...
    auto started = std::chrono::high_resolution_clock::now();
    // generate multiple threads for access data in parallel
    
    for (int i = 0; i < NUM_THREADS; i++) {
      threadInfo[i].thread_id = i;
      threadInfo[i].job_size = s / NUM_THREADS;
      err = pthread_create(&thread[i], NULL, thr_access_fn, (void*)&threadInfo[i]);
      if (err != 0) {
        printf("error during thread creation, exit...\n");
        exit(-1);
      }
    ...
    /**
       for(int i = 0; i < NUM_NODES; i++) {
       if (i == NUM_NODES -1) {
       s_each = s_each + s % NUM_NODES;
       }
       for (int j = 0; j < s_each; j++) {
       printf("%.1f ", mx[i][j]);
       }
       printf("\n");
       }
    ...
    for (int i = 0; i < NUM_NODES; i++) {
      numa_free(mx[i], s_each * sizeof(float));
    }
    free(threadInfo);
    free(mx);
    free(thread);
  ...
  void* thr_alloc_fn(void* arg) {
    ...
    //  printf("pid = %lu, tid = %lu, for i = %d, is running on cpu: %d\n", (unsigned long)pid, (unsigned long)tid, i, sched_getcpu());
    mx[i] = (float*)numa_alloc_onnode(s_each * sizeof(float), i);
    
    for (int k = 0; k < s_each; k++) {
      mx[i][k] = 0.0;
    }
  ...
  void* thr_access_fn(void* arg) {
    ...
    // check if the corresponding thread is running on the right set of CPUs, so it has local access to corresponding node.
    node_index = id / (NUM_THREADS / NUM_NODES);
    // migrate thread to certain cpu set, so that it can access data locally
    if (node_index != current_on_node) {
      //     printf("pid = %lu, tid = %ld, on cpu: %d, node_index: %d, current_on_node: %d\n", (unsigned long)pid, (unsigned long)info.thread_id, sched_cpu, node_index, current_on_node);
      // create cpu set mask
      if (!without_numa) {
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
      ...
      //    printf("pid = %lu, tid = %ld, on cpu: %d, node_index: %d\n", (unsigned long)pid, (unsigned long)info.thread_id, sched_cpu, node_index);
    }
    // p tells the current thread's postion in the corresponding node section
    int p = id - node_index * (NUM_THREADS / NUM_NODES);

    int from = p * info.job_size;
    int to = from + info.job_size;
    // if it is the last thread in a NUMA node section
    if ((p + 1) % (NUM_THREADS / NUM_NODES) == 0) {
      to = s_each % (NUM_THREADS / NUM_NODES) + to;
    }
