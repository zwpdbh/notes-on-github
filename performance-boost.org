...
#+BEGIN_SRC c
  ...
  int main(int argc, char* argv[]) {
    ...
    s = m * n;
    if (s % NUM_THREADS) {
      printf("s % NUM_THREADS = %d, it should be = 0, exit...\n", s % NUM_THREADS);
    } else {
      printf("matrix size = %ld * %ld = %ld\n", m, n, s);
    }
  ...
  void* thr_access_fn(void* arg) {
    ...
    pid = getpid();
    which_cpu = sched_getcpu();

    int id = info.thread_id;

    printf("pid = %lu, tid = %ld, is running on cpu: %d, on node: %d ", (unsigned long)pid, (unsigned long)info.thread_id, which_cpu, topology[which_cpu]);
    
    // check if the corresponding thread is running on the right set of CPUs, so it has local access to corresponding node.
    which_node = id / (NUM_THREADS / NUM_NODES);
    ...
    int p = id - which_node * (NUM_THREADS / NUM_NODES);
    printf("thread %d, p = %d ", id, p);
