...
#+BEGIN_SRC C
  ...
  int main(int argc, char* argv[]) {
    ...
    auto done = std::chrono::high_resolution_clock::now();
    std::cout << "parallel access matrix, use: " << std::chrono::duration_cast<std::chrono::nanoseconds>(done-started).count() << "ns" << endl;
    printf("\n");
    //  check_mx();
    ...
    std::cout << "parallel multiplication, use: " << std::chrono::duration_cast<std::chrono::nanoseconds>(done-started).count() << "ns" << endl;
    //check_w();
...
*** Execution record
=./bin/parallel_memory_access 100000 100000 0= for with numa awareness
=./bin/parallel_memory_access 100000 100000 1= for without numa awareness

|   id |        with |     without |
|    1 |  1123583304 |  1196886063 |
|    2 |  1027846365 |  1213154223 |
|    3 |   961966382 |  1197822674 |
|    4 |  1001347748 |  1190312127 |
| mean | 1028685900. | 1199543800. |
#+TBLFM: @6$2=vmean(@2..@5)::@6$3=vmean(@2..@5)

- result with matrix time vector
  #+BEGIN_SRC sh
    wzhao@r815:~/numa-knn$ ./bin/matrix_multiplication 128000 128000 1
    WITH numa
    s = 16384000000
    allocate 128000 by 128000 matrix
    parallel access matrix, use: 2681891576ns

    parallel multiplication, use: 23441440153ns
    wzhao@r815:~/numa-knn$ ./bin/matrix_multiplication 128000 128000 0
    WITHOUT numa configuration
    s = 16384000000
    allocate 128000 by 128000 matrix
    parallel access matrix, use: 2843614110ns

    parallel multiplication, use: 23304168441ns
    wzhao@r815:~/numa-knn$ ./bin/matrix_multiplication 200000 200000 1
    WITH numa
    s = 40000000000
    allocate 200000 by 200000 matrix
    parallel access matrix, use: 6593969799ns

    parallel multiplication, use: 58298070880ns
    wzhao@r815:~/numa-knn$ ./bin/matrix_multiplication 200000 200000 0
    WITHOUT numa configuration
    s = 40000000000
    allocate 200000 by 200000 matrix
    parallel access matrix, use: 6545678079ns

    parallel multiplication, use: 58185872610ns
    wzhao@r815:~/numa-knn$ ./bin/matrix_multiplication 200000 200000 1
    WITH numa
    s = 40000000000
    allocate 200000 by 200000 matrix
    parallel access matrix, use: 6576542917ns

    parallel multiplication, use: 58374621968ns
    wzhao@r815:~/numa-knn$ ./bin/matrix_multiplication 200000 200000 0
    WITHOUT numa configuration
    s = 40000000000
    allocate 200000 by 200000 matrix
    parallel access matrix, use: 6849083205ns

    parallel multiplication, use: 58467123391ns
  #+END_SRC

- time measurement with PMC
  size = 128000 * 128000  and 200000 * 200000  
  | id                                 |             1 |             2 |             3 |             4 |
  | access with NUMA in PMC            |    5866552998 |    5609193649 |   15030122505 |   13276063893 |
  | access with NUMA                   |  2550633016ns |  2438739966ns |  6534770741ns |  5772144665ns |
  | multiplication with NUMA in PMC    |   53725628910 |   53966388707 |  133871754025 |  173081438682 |
  | multiplication with NUMA           | 23358808022ns | 23463484569ns | 58204727318ns | 75252300364ns |
  | access without NUMA in PMC         |    5448400375 |    5395319446 |   12840327576 |   12799387211 |
  | access wihout NUMA                 |  2368826995ns |  2345749596ns |  5582692694ns |  5564883155ns |
  | multiplication without NUMA in PMC |   53763292569 |   53476715266 |  133370890684 |  134135194494 |
  | multiplication without NUMA        | 23375182822ns | 23250584307ns | 57986960759ns | 58319263048ns |
